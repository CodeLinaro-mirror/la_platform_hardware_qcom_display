...
DisplayError SetCSC(const native_handle_t *handle, ColorMetaData *color_metadata) {
  ...
  if (qtigralloc::getMetadataState(const_cast<native_handle_t *>(handle), QTI_COLOR_METADATA)) {
    ...
    if (!err) {
      ...
      color_metadata->matrixCoefficients = color.matrixCoefficients;
...
HWC3::Error HWCLayer::SetLayerBuffer(buffer_handle_t buffer, int32_t acquire_fence) {
  if (!buffer) {
    if (client_requested_ == Composition::DEVICE ||
        client_requested_ == Composition::CURSOR) {
  ...
  bool secure = (flags & qtigralloc::PRIV_FLAGS_SECURE_BUFFER);
  bool secure_camera = secure && (flags & qtigralloc::PRIV_FLAGS_CAMERA_WRITE);
