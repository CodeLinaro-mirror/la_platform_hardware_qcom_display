...
void ComposerHandleImporter::InoFdMapInsert(int fd) {
  ...
  ALOGV("insert fd=%d, ino=%llu", fd, ino);
  ino_fds_map_[ino].push_back(fd);
  if (ino_fds_map_.size() > MAX_INO_VALS) {
    ALOGW("ino allocation count=%zu", ino_fds_map_.size());
  }
}
...
void ComposerHandleImporter::InoFdMapRemove(int fd) {
  ...
  if (it == fds->end()) {
    ALOGW("Ino value not found! Should not happen. ino=%llu, size=%zu", ino, fds->size());
    return;
  }
  ALOGV("remove fd=%d, ino=%llu", fd, ino);
  fds->erase(it);
  if (!ino_fds_map_[ino].size()) {
    ino_fds_map_.erase(ino);
  }
  if (ino_fds_map_.size() > MAX_INO_VALS) {
    ALOGW("allocation count=%zu", ino_fds_map_.size());
  }
}
