...
HWC3::Error HWCDisplay::SetActiveConfig(Config config) {
  ...
  if (IsVirtualConfig(config) || IsVirtualConfig(active_config_index_)) {
    HWC3::Error error = SetFBForExtendedResolution(config, &real_config_for_fps_switch);
...
DisplayError HWCDisplay::VSync(const DisplayEventVSync &vsync) {
  ...
  if (GetDisplayVsyncPeriod(false, &vsync_period) != HWC3::Error::None) {
    vsync_period = 0;
  }
...
HWC3::Error HWCDisplay::GetDisplayVsyncPeriod(bool get_real_config, VsyncPeriodNanos *vsync_period) {
...
HWC3::Error HWCDisplay::SetActiveConfigWithConstraints(
  ...
  VsyncPeriodNanos vsync_period;
  if (GetDisplayVsyncPeriod(true, &vsync_period) != HWC3::Error::None) {
    return HWC3::Error::BadConfig;
  }
...
void HWCDisplay::ProcessActiveConfigChange() {
  ...
  DTRACE_SCOPED();
  VsyncPeriodNanos vsync_period;
  if (GetVsyncPeriodByActiveConfig(true, &vsync_period) == HWC3::Error::None) {
    SubmitActiveConfigChange(vsync_period);
  }
}

HWC3::Error HWCDisplay::GetVsyncPeriodByActiveConfig(bool get_real_config,
                                                     VsyncPeriodNanos *vsync_period) {
...
HWC3::Error HWCDisplay::GetCachedActiveConfig(bool get_real_config, Config *active_config) {
...
HWC3::Error HWCDisplay::SetFBForExtendedResolution(hwc2_config_t config,
