...
void HWCDebugger::Log(DebugLogType type, const char *log_tag, const char *fmt, std::va_list &args) {
  auto log_type = log_type_map_[type];
  __android_log_vprint(log_type, log_tag, fmt, args);
}

int HWCDebugger::GetProperty(const char *property_name, int *value) {
...
int HWCDebugger::GetProperty(const char *property_name, char *value) {
