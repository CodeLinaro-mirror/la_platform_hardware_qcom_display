...
Error BufferManager::ValidateBufferSize(private_handle_t const *hnd, BufferInfo info) {
  ...
  auto ion_fd_size = static_cast<unsigned int>(
      lseek(const_cast<private_handle_t *>(hnd)->fd(), 0, SEEK_END));
  if (size != ion_fd_size) {
    return Error::BAD_VALUE;
  }
  return Error::NONE;
...
void BufferManager::RegisterHandleLocked(const private_handle_t *hnd, int ion_handle,
  ...
  auto handle = const_cast<private_handle_t *>(hnd);
  if (handle->base_metadata()) {
#ifdef METADATA_V2
    buffer->reserved_size = handle->reserved_size();
    if (buffer->reserved_size > 0) {
      buffer->reserved_region_ptr =
...
Error BufferManager::ReleaseBuffer(private_handle_t const *hnd) {
  ...
  if (buf == nullptr) {
    ...
    if (buf->DecRef()) {
      handles_map_.erase(hnd);
      // Unmap, close ion handle and close fd
      if (allocated_ >= const_cast<private_handle_t *>(hnd)->size()) {
        allocated_ -= const_cast<private_handle_t *>(hnd)->size();
      }
      FreeBuffer(buf);
    }
  }
...
Error BufferManager::LockBuffer(const private_handle_t *hnd, uint64_t usage) {
  ...
  // todo use handle here
  if (err == Error::NONE && (handle->flags() & private_handle_t::PRIV_FLAGS_USES_ION) &&
      (handle->flags() & private_handle_t::PRIV_FLAGS_CACHED)) {
    if (allocator_->CleanBuffer(reinterpret_cast<void *>(handle->base()),
                                handle->size(), handle->offset(),
                                buf->ion_handle_main, CACHE_INVALIDATE, handle->fd())) {
      return Error::BAD_BUFFER;
    }
  }
...
Error BufferManager::AllocateBuffer(AllocData *ad, AllocData *m_data, uint64_t usage,
                                    int format) {
  auto err = allocator_->AllocateMem(ad, usage, format);
  if (err) {
    ALOGE("gralloc failed to allocate err=%s format %d size %d usage %" PRIu64,
          strerror(-err), format, ad->size, usage);
    return Error::NO_RESOURCES;
  }

  err = allocator_->AllocateMem(m_data, 0, 0);
  if (err) {
    ALOGE("gralloc failed to allocate metadata error=%s", strerror(-err));
    return Error::NO_RESOURCES;
  }
...
Error BufferManager::AllocateBuffer(const BufferDescriptor &descriptor, buffer_handle_t *handle,
  ...
  auto alloc_err = AllocateBuffer(&data, &e_data, usage, format);

  if (alloc_err != Error::NONE) {
    return alloc_err;
  }
  ...
  if (usage & GRALLOC_USAGE_PRIVATE_MULTIVIEW) {
    ...
    alloc_err = AllocateBuffer(&data_2, &e_data_2, usage, format);
    if (alloc_err != Error::NONE) {
      return alloc_err;
    }
...
Error BufferManager::GetViewToImport(private_handle_t *handle, const uint32_t view_requested,
  ...
  if (!metadata->isVendorMetadataSet[GET_VENDOR_METADATA_STATUS_INDEX(QTI_VIEW_ID)] ||
  ...
  uint32_t left_id_from_sei = (metadata->threeDimensionalRefInfo.threedRefDispInfo[0]).left_view_id;
  uint32_t right_id_from_sei =
      (metadata->threeDimensionalRefInfo.threedRefDispInfo[0]).right_view_id;
  ...
  if (view_id_from_metadata == left_id_from_sei) {
    view_at_index_0 = handle->view();
  } else {
    view_at_index_0 = handle->getViewInfo() & (~handle->view());
  }

  if (view_requested == view_at_index_0) {
    *view = handle->view();
  } else {
    *view = handle->getViewInfo() & (~handle->view());
  }

  ALOGD_IF(
      DEBUG,
      "view_requested %d view_id_from_metadata %d , left_id_from_sei %d, right_id_from_sei %d, "
      "view_at_index_0 %d, view returned %d",
      view_requested, view_id_from_metadata, left_id_from_sei, right_id_from_sei, view_at_index_0,
      *view);
...
#ifdef QTI_BASE_VIEW
    case QTI_BASE_VIEW: {
      android::gralloc4::encodeUint32(qtigralloc::MetadataType_BaseView, handle->view(), out);
      break;
    }
#endif
#ifdef QTI_MULTI_VIEW_INFO
    case QTI_MULTI_VIEW_INFO: {
      android::gralloc4::encodeUint32(qtigralloc::MetadataType_MultiViewInfo,
                                      handle->getViewInfo(), out);
      break;
    }
#endif
#ifdef QTI_THREE_DIMENSIONAL_REF_INFO
