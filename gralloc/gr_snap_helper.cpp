...
int32_t Mapper5Encode(const typename StandardMetadata<T>::value_type &value, void *out_buffer,
  ...
  if (size_required < 0) {
    ALOGW_IF(-AIMAPPER_ERROR_UNSUPPORTED != size_required,
             "%s: Unexpected error %" PRId32 " during size calculation for encode (%" PRId64 ") call", __FUNCTION__,
  ...
  if (out_buffer != nullptr && size_required <= out_size) {
    ...
    if (size_required < 0 || (size_t)size_required > out_size) {
      ALOGW("Mapper5Encode (%" PRId64 ") failed, calculated size %" PRId32 " with buffer size %zd",
...
SnapError GrallocSnapHelper::PPParamInterlacedHelper(SnapHandle *hnd, uint32_t aidl_size,
  ...
  auto error = SnapError::BAD_VALUE;
  if (gralloc_out_get != nullptr) {
    error = snapmapper_->GetMetadata(*hnd, SnapMetadataType::MD_PP_PARAM_INTERLACED, gralloc_out_get);
  } else if (gralloc_in_set != nullptr) {
    error = snapmapper_->SetMetadata(*hnd, SnapMetadataType::MD_PP_PARAM_INTERLACED, gralloc_in_set);
  }
  return error;
}
...
SnapError GrallocSnapHelper::MapSecureBufferHelper(SnapHandle *hnd, uint32_t aidl_size,
  ...
  auto error = SnapError::BAD_VALUE;
  if (gralloc_out_get != nullptr) {
    error = snapmapper_->GetMetadata(*hnd, SnapMetadataType::MD_MAP_SECURE_BUFFER, gralloc_out_get);
  } else if (gralloc_in_set != nullptr) {
    error = snapmapper_->SetMetadata(*hnd, SnapMetadataType::MD_MAP_SECURE_BUFFER, gralloc_in_set);
  }
  return error;
}
...
SnapError GrallocSnapHelper::LinearFormatHelper(SnapHandle *hnd, uint32_t aidl_size,
  ...
  auto error = SnapError::BAD_VALUE;
  if (gralloc_out_get != nullptr) {
    error = snapmapper_->GetMetadata(*hnd, SnapMetadataType::MD_LINEAR_FORMAT, gralloc_out_get);
  } else if (gralloc_in_set != nullptr) {
    error = snapmapper_->SetMetadata(*hnd, SnapMetadataType::MD_LINEAR_FORMAT, gralloc_in_set);
  }
  return error;
}
  ...
  if (hnd != nullptr) {
    ...
    if (metadata_conversion_helper_function_map.find(snap_metadata_type) !=
      ...
      if (error == SnapError::METADATA_NOT_SET && !check_metadata_set) {
        ALOGI("Metadata type %" PRIu64 " is not set.Returning default values as check_metadata_set is %d",
...
int GrallocSnapHelper::ConvertSnapBufferlayoutToGrallocPlaneLayout(
  ...
  for (int i = 0; i < plane_count; i++) {
    ...
    ALOGD_IF(
        enable_logs_,
        "Plane No: %d, sampleIncrementInBits %" PRId64 ", strideInBytes %" PRId64 ", totalSizeInBytes %" PRId64 ", "
        "horizontalSubsampling %" PRId64 ", verticalSubsampling %" PRId64 ", widthInSamples %" PRId64 ",  heightInSamples %" PRId64 ", "
        "offsetInBytes %" PRId64,
...
int GrallocSnapHelper::ConvertSnapDataspaceToGrallocDataspace(SnapDataspace &snap_dataspace,
  ...
  switch (snap_dataspace.colorPrimaries) {
    ...
    case QtiColorPrimaries_DCIP3:
      ...
      break;
    default:
      ALOGD_IF(enable_logs_,"%s: Failed to convert primaries %d",
               __FUNCTION__, snap_dataspace.colorPrimaries);
      return SnapError::BAD_VALUE;
  }
  ...
  switch (snap_dataspace.transfer) {
    ...
    case QtiTransfer_SMPTE_ST2084:
      ...
      break;
    default:
      ALOGW("%s: Failed to convert transfer %d",
             __FUNCTION__, snap_dataspace.transfer);
      return SnapError::BAD_VALUE;
  }
  ...
  switch (snap_dataspace.range) {
    ...
    case QtiRange_Extended:
      ...
      break;
    default:
      ALOGW("%s: Failed to convert range %d",
             __FUNCTION__, snap_dataspace.range);
      return SnapError::BAD_VALUE;
  }
...
SnapError GrallocSnapHelper::GetSnapDescriptor(gralloc::BufferDescriptor gr_desc,
  ...
  if (err) {
    ...
    snap_desc.additionalOptions.emplace_back(modifier);
    ALOGD_IF(enable_logs_,
             "GetSnapDescriptor gr format %d gr usage %" PRIu64 " snap format %d snap modifier %d snap "
             "usage %" PRIu64,
...
SnapError GrallocSnapHelper::GetSnapDescriptor(gralloc::BufferInfo gr_desc,
  ...
  if (err) {
    ...
    if (err) {
      ALOGW("Error while getting snap descriptor - Unknown Usage bit set - %" PRIu64, gr_desc.usage);
      return err;
    }
...
int GrallocSnapHelperLegacy::GetMetadata(native_handle_t *gr_hnd, uint64_t gr_metadata_type,
  ...
  if (hnd != nullptr) {
    ...
    if (metadata_conversion_helper_function_map.find(snap_metadata_type) !=
      ...
      if (error == SnapError::METADATA_NOT_SET && !check_metadata_set) {
        ALOGI("Metadata type %" PRId64 " is not set.Returning default values as check_metadata_set is %d",
              gr_metadata_type, check_metadata_set);
        return SnapError::NONE;
      }
      return error;
...
int GrallocSnapHelperLegacy::ConvertSnapDataspaceToGrallocDataspace(
  ...
  switch (snap_dataspace.transfer) {
    ...
    case QtiTransfer_SMPTE_ST2084:
      ...
      break;
    default:
      ALOGD_IF(enable_logs_,"%s: Failed to convert transfer %d",
                 __FUNCTION__, snap_dataspace.transfer);
      return SnapError::BAD_VALUE;
  }
  ...
  switch (snap_dataspace.range) {
    ...
    case QtiRange_Extended:
      ...
      break;
    default:
      ALOGD_IF(enable_logs_,"%s: Failed to convert range %d",
               __FUNCTION__, snap_dataspace.range);
      return SnapError::BAD_VALUE;
  }
