...
static NativeHandle AIDLNativeHandleFromSnapHandle(SnapHandle *snap_buffer_handle, bool pass_fd_ownership) {
...
static SnapHandle *SnapHandleFromCNativeHandle(native_handle_t *native_handle, bool pass_fd_ownership) {
  if (!native_handle)
    return nullptr;
...
SnapError GrallocSnapHelper::CheckMetadataSet(SnapMetadataType type, SnapError status,
...
SnapError GrallocSnapHelper::PixelFormatRequestedHelper(SnapHandle *hnd, bool hidl_bytestream,
  ...
  if (aidl_size) {
    ...
    *static_cast<PixelFormat_V1_2 *>(gralloc_out_get) = static_cast<PixelFormat_V1_2>(gr_format);
  }
  return error;
}
...
int GrallocSnapHelper::SetMetadata(native_handle_t *gr_hnd, uint64_t gr_metadata_type,
                                   hidl_vec<uint8_t> in) {
  if (gr_hnd == nullptr) {
    ALOGE("Invalid gralloc handle");
    return SnapError::BAD_BUFFER;
...
int GrallocSnapHelper::GetGrallocPrivateFlags(SnapUsage snap_usage, int64_t is_ubwc,
