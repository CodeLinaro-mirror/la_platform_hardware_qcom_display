...
namespace std {
  ...
  template<>
  struct hash<SnapMetadataType> {
    using argument_type = SnapMetadataType;
    using result_type = std::size_t;
...
class GrallocSnapHelper {
 ...
 private:
  ...
  std::unordered_map<SnapFormatDescriptor, uint64_t, SnapFormatDescriptorHash>
      snap_to_gralloc_format_ = {
          ...
          {{.format = SnapPixelFormat::YCrCb_420_SP, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::YCRCB_420_SP)},
          ...
          {{.format = SnapPixelFormat::RGBA_1010102, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::RGBA_1010102)},
          ...
          {{.format = SnapPixelFormat::IMPLEMENTATION_DEFINED,
           ...
           static_cast<int>(GrallocPixelFormat::IMPLEMENTATION_DEFINED)},
          {{.format = SnapPixelFormat::YCBCR_420_888, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::YCBCR_420_888)},
          ...
          {{.format = SnapPixelFormat::RAW_OPAQUE, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::RAW_OPAQUE)},
          {{.format = SnapPixelFormat::YCBCR_422_I, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::YCBCR_422_I)},
          {{.format = SnapPixelFormat::IMPLEMENTATION_DEFINED,
            .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::IMPLEMENTATION_DEFINED)},
          {{.format = SnapPixelFormat::IMPLEMENTATION_DEFINED,
          ...
          {{.format = SnapPixelFormat::Y8, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::Y8)},
          {{.format = SnapPixelFormat::Y16, .modifier = PIXEL_FORMAT_MODIFIER_NONE},
           static_cast<int>(GrallocPixelFormat::Y16)},
  ...
  std::unordered_map<uint64_t, SnapMetadataType> metadata_type_map = {
    ...
    {static_cast<uint64_t>(StandardMetadataType::STRIDE), SnapMetadataType::STRIDE},
    ...
    {QTI_PP_PARAM_INTERLACED, SnapMetadataType::MD_PP_PARAM_INTERLACED},
    //{QTI_PP_PARAM_INTERLACED, PP_PARAM_INTERLACED},
    {QTI_REFRESH_RATE, SnapMetadataType::REFRESH_RATE},
    {QTI_MAP_SECURE_BUFFER, SnapMetadataType::MD_MAP_SECURE_BUFFER},
    //{QTI_MAP_SECURE_BUFFER, MAP_SECURE_BUFFER},
    {QTI_LINEAR_FORMAT, SnapMetadataType::MD_LINEAR_FORMAT},
  ...
  std::unordered_map<uint64_t, MetadataHelper>
      metadata_conversion_helper_function_map = {
          {static_cast<uint64_t>(SnapMetadataType::BUFFER_ID), &GrallocSnapHelper::BufferIDHelper},
          {static_cast<uint64_t>(SnapMetadataType::NAME), &GrallocSnapHelper::NameHelper},
          {static_cast<uint64_t>(SnapMetadataType::WIDTH), &GrallocSnapHelper::WidthHelper},
          {static_cast<uint64_t>(SnapMetadataType::HEIGHT), &GrallocSnapHelper::HeightHelper},
          {static_cast<uint64_t>(SnapMetadataType::LAYER_COUNT), &GrallocSnapHelper::LayerCountHelper},
          {static_cast<uint64_t>(SnapMetadataType::PIXEL_FORMAT_REQUESTED), &GrallocSnapHelper::PixelFormatRequestedHelper},
          {static_cast<uint64_t>(SnapMetadataType::PIXEL_FORMAT_FOURCC), &GrallocSnapHelper::PixelFormatFourCCHelper},
          {static_cast<uint64_t>(SnapMetadataType::DRM_PIXEL_FORMAT_MODIFIER), &GrallocSnapHelper::DRMPixelFormatModifierHelper},
          {static_cast<uint64_t>(SnapMetadataType::USAGE), &GrallocSnapHelper::UsageHelper},
          {static_cast<uint64_t>(SnapMetadataType::ALLOCATION_SIZE), &GrallocSnapHelper::AllocationSizeHelper},
          {static_cast<uint64_t>(SnapMetadataType::PROTECTED_CONTENT), &GrallocSnapHelper::ProtectedContentHelper},
          {static_cast<uint64_t>(SnapMetadataType::COMPRESSION), &GrallocSnapHelper::CompressionHelper},
          {static_cast<uint64_t>(SnapMetadataType::INTERLACED), &GrallocSnapHelper::InterlacedHelper},
          {static_cast<uint64_t>(SnapMetadataType::CHROMA_SITING), &GrallocSnapHelper::ChromaSitingHelper},
          {static_cast<uint64_t>(SnapMetadataType::PLANE_LAYOUTS), &GrallocSnapHelper::PlaneLayoutsHelper},
          {static_cast<uint64_t>(SnapMetadataType::CROP), &GrallocSnapHelper::CropHelper},
          {static_cast<uint64_t>(SnapMetadataType::DATASPACE), &GrallocSnapHelper::DataspaceHelper},
          {static_cast<uint64_t>(SnapMetadataType::BLEND_MODE), &GrallocSnapHelper::BlendModeHelper},
          {static_cast<uint64_t>(SnapMetadataType::VT_TIMESTAMP), &GrallocSnapHelper::VTTimestampHelper},
          {static_cast<uint64_t>(SnapMetadataType::MD_PP_PARAM_INTERLACED), &GrallocSnapHelper::PPParamInterlacedHelper},
          {static_cast<uint64_t>(SnapMetadataType::VIDEO_PERF_MODE), &GrallocSnapHelper::VideoPerfModeHelper},
          {static_cast<uint64_t>(SnapMetadataType::GRAPHICS_METADATA), &GrallocSnapHelper::GraphicsMetadataHelper},
          {static_cast<uint64_t>(SnapMetadataType::UBWC_CR_STATS_INFO), &GrallocSnapHelper::UBWCCRStatsInfoHelper},
          {static_cast<uint64_t>(SnapMetadataType::REFRESH_RATE), &GrallocSnapHelper::RefreshRateHelper},
          {static_cast<uint64_t>(SnapMetadataType::MD_MAP_SECURE_BUFFER), &GrallocSnapHelper::MapSecureBufferHelper},
          {static_cast<uint64_t>(SnapMetadataType::MD_LINEAR_FORMAT), &GrallocSnapHelper::LinearFormatHelper},
          {static_cast<uint64_t>(SnapMetadataType::SINGLE_BUFFER_MODE), &GrallocSnapHelper::SingleBufferModeHelper},
          {static_cast<uint64_t>(SnapMetadataType::CVP_METADATA), &GrallocSnapHelper::CVPMetadataHelper},
          {static_cast<uint64_t>(SnapMetadataType::VIDEO_HISTOGRAM_STATS), &GrallocSnapHelper::VideoHistogramStatsHelper},
          {static_cast<uint64_t>(SnapMetadataType::FD), &GrallocSnapHelper::FDHelper},
          {static_cast<uint64_t>(SnapMetadataType::ALIGNED_WIDTH_IN_PIXELS), &GrallocSnapHelper::AlignedWidthInPixelsHelper},
          {static_cast<uint64_t>(SnapMetadataType::STRIDE), &GrallocSnapHelper::AlignedWidthInPixelsHelper},
          {static_cast<uint64_t>(SnapMetadataType::ALIGNED_HEIGHT_IN_PIXELS), &GrallocSnapHelper::AlignedHeightInPixelsHelper},
          {static_cast<uint64_t>(SnapMetadataType::STANDARD_METADATA_STATUS), &GrallocSnapHelper::StandardMetadataStatusHelper},
          {static_cast<uint64_t>(SnapMetadataType::VENDOR_METADATA_STATUS), &GrallocSnapHelper::VendorMetadataStatusHelper},
          {static_cast<uint64_t>(SnapMetadataType::BUFFER_TYPE), &GrallocSnapHelper::BufferTypeHelper},
          {static_cast<uint64_t>(SnapMetadataType::VIDEO_TS_INFO), &GrallocSnapHelper::VideoTSInfoHelper},
          {static_cast<uint64_t>(SnapMetadataType::CUSTOM_DIMENSIONS_STRIDE), &GrallocSnapHelper::CustomDimensionsStrideHelper},
          {static_cast<uint64_t>(SnapMetadataType::CUSTOM_DIMENSIONS_HEIGHT), &GrallocSnapHelper::CustomDimensionsHeightHelper},
          {static_cast<uint64_t>(SnapMetadataType::RGB_DATA_ADDRESS), &GrallocSnapHelper::RGBDataAddressHelper},
          {static_cast<uint64_t>(SnapMetadataType::BUFFER_PERMISSION), &GrallocSnapHelper::BufferPermissionHelper},
          {static_cast<uint64_t>(SnapMetadataType::MEM_HANDLE), &GrallocSnapHelper::MemHandleHelper},
          {static_cast<uint64_t>(SnapMetadataType::TIMED_RENDERING), &GrallocSnapHelper::TimedRenderingHelper},
          ...
          {static_cast<uint64_t>(SnapMetadataType::BASE_ADDRESS), &GrallocSnapHelper::BaseAddressHelper},
          {static_cast<uint64_t>(SnapMetadataType::MATRIX_COEFFICIENTS), &GrallocSnapHelper::MatrixCoefficientsHelper},
          {static_cast<uint64_t>(SnapMetadataType::EARLYNOTIFY_LINECOUNT), &GrallocSnapHelper::EarlyNotifyLineCountHelper},
  };
