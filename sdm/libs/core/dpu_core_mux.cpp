...
DisplayError DPUCoreMux::SetPPFeatures(PPFeaturesConfig *feature_list) {
  for (auto hw_intf : hw_intf_) {
     DisplayError error = hw_intf->SetPPFeatures(feature_list);
     if (error != kErrorNone) {
       return error;
     }
  }
...
DisplayError DPUCoreMux::SetDimmingConfig(void *payload, size_t size) {
  for (auto hw_intf : hw_intf_) {
     DisplayError error = hw_intf->SetDimmingConfig(payload, size);
     if (error != kErrorNone) {
       return error;
     }
  }
...
DisplayError DPUCoreMux::EnableSelfRefresh() {
  for (auto hw_intf : hw_intf_) {
     DisplayError error = hw_intf->EnableSelfRefresh();
     if (error != kErrorNone) {
       return error;
     }
  }
